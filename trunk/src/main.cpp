...
int main(int argc, char **argv)
{
    ...
    if (argc == 3) {
        ...
        if (not strcmp(argv[1], "-cli"))
            return Cli::the().Run(QString::fromUtf8(argv[2]));
        if (not strcmp(argv[2], "-cli"))
            return Cli::the().Run(QString::fromUtf8(argv[1]));
    }
	
    cerr << "setting up application... ";
#if defined(Q_WS_X11) && defined(AUTOTYPE)
    cerr << "(KeepassApplication) ";
...
bool CmdLineArgs::parse(const QStringList& argv){
	for(int i=1;i<argv.size();i++){
		...
		Error = QString("** Unrecognized argument: '%1'. You should pass filename as the first argument, if present.").arg(argv[i]);
		return false;
	}
	return true;
}
